...
async function main() {
    ...
    const samurai = await Samurai.deploy();
    await samurai.deployed();
    console.log("Samurai contract deployed to: ", samurai.address);

    const ConquestCoin = await hre.ethers.getContractFactory("ConquestCoin");
    const conquestCoin = await ConquestCoin.deploy();
    await conquestCoin.deployed();
    console.log("ConquestCoin contract deployed to: ", conquestCoin.address);

    const Game = await hre.ethers.getContractFactory("Game");
    const game = await Game.deploy(samurai.address, conquestCoin.address);
    await game.deployed();
    console.log("Game contract deployed to: ", game.address);


    let config = `module.exports = {
        samuraiaddress : "${samurai.address}",
        conquestcoinaddress : "${conquestCoin.address}",
  ...
  `
    ...
    const gameContract = await ethers.getContractAt("Game", game.address);
    ...
    await gameContract.addLand(30, 0, 5, false)
